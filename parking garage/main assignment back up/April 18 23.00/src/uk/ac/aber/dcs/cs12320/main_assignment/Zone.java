...
public class Zone {
    ...
    private void loadZoneSpace(boolean[] array) throws IOException{
        FileReader fileInput = new FileReader("price.txt");
        Scanner infile = new Scanner(fileInput);
        infile.useDelimiter(":|\r?\n|\r");
        for (int i = 0; i < array.length; i++){
            boolean s = infile.nextBoolean();
            array[i] = s;
        }
    }
    private void saveZoneSpace(boolean[] array) throws IOException{
    ...
    void load() throws IOException {
        ...
        Scanner infile = new Scanner(fileInput);
        infile.useDelimiter(":|\r?\n|\r");
        loadZoneSpace(z1Space);
        loadZoneSpace(z2Space);
        loadZoneSpace(z3Space);
        loadZoneSpace(z3Space);
        loadZoneSpace(z5Space);
    ...
    void save() throws IOException {
        FileWriter fileOutput = new FileWriter("price.txt");
        PrintWriter outfile = new PrintWriter(fileOutput);
        saveZoneSpace(z1Space);
        saveZoneSpace(z2Space);
        saveZoneSpace(z3Space);
        saveZoneSpace(z4Space);
        saveZoneSpace(z5Space);
        for (Zone z:zones) {
