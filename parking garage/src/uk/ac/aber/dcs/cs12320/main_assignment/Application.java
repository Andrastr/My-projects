...
public class Application {
    ...
    private void printMenu(){
        ...
        System.out.format(format, "q. Save and quit");
        System.out.format(format, "");
        System.out.println("+ ---------------------------------- +");
    }
    ...
    private void userMenu() throws InterruptedException {
        ...
        do {
            ...
            switch (response) {
                case "1":   //checks if there is a free attendant, if there is, prints the attendant menu
                    boolean help = garage.callAttendant();
                    if (help) attendantMenu();
                    break;
                case "2":   //Registers a vehicle
                    garage.registerVehicle(false);
                    break;
                case "3":   //Retrieves a vehicle
                    garage.retrieveVehicle();
                    break;
                case "4":   //Prints the garage zones
                    garage.displayZoneMenu();
                    pressEnterToContinue();
                    break;
                case"5":    //Prints the admin menu
                    System.out.println("\n\n\n\n\n\n\n\n\n\n");
                    adminMenu();
                    break;
                case"6":    //Prints all ArrayLists and arrays
    ...
    private void attendantMenu() throws InterruptedException {
        ...
        do{
            ...
            response = scan.nextLine().toUpperCase();
            switch(response){
                case "1":   //Registers a vehicle
                    garage.registerVehicle(true);
                    quit = true;
                    break;
                case "2":   //Retrieves a vehicle
                    garage.retrieveVehicle();
                    quit = true;
                    break;
                case"3":    //Display the parking spaces of each zone
                    for (int i = 1; i<6; i++){
    ...
    private void adminMenu(){
        ...
        do{
            ...
            response = scan.nextLine().toUpperCase();
            switch(response){
                case "0":   //Creates a new attendant
                    garage.newAttendant();
                    break;
                case "1":   //Removes an attendants
                    garage.editAttendant(1);
                    break;
                case "2":   //Changes an attendant salary
                    garage.editAttendant(2);
                    break;
                case "3":   //Sets all attendants as free
                    garage.changeAttendants(1);
                    break;
                case "4":   //Sets all attendants as occupied
                    garage.changeAttendants(2);
                    break;
                case "5":   //Prints all attendants, first free then occupied
                    System.out.println("\n\n---Free Attendants---");
                    garage.printAttendants(1);
                    System.out.println("\n---Occupied Attendants---");
                    garage.printAttendants(2);
                    pressEnterToContinue();
                    break;
                case "6":   //Change the price of a zone in the garage
                    garage.changePrices();
                    break;
                case "7":   //Print the ArrayList zones
                    garage.printZones();
                    pressEnterToContinue();
                    break;
                case "8":   //Display the parking spaces of each zone
                    for (int i = 1; i<6; i++){
                        garage.displayZone(i);
                    }
                    pressEnterToContinue();
                    break;
                case "9":   //exits the admin menu
                    quit = true;
                    break;
                default:
                    System.out.println("\n\n\n\n\nInvalid input, try again\n");
